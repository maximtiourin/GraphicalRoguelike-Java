...
public class TileSet {
	...
	public TileSet(TiledMap map, Element element, boolean loadImage) throws SlickException {
		...
		if (loadImage) {
			String path=map.getTilesLocation()+"/"+ref;
			while (path.contains("/..")){
				int dds=path.lastIndexOf("/..");
				String beforeThat=path.substring(0, dds);
				String afterThat=path.substring(dds+3);
				path=beforeThat.substring(0,beforeThat.lastIndexOf("/"))+afterThat;
			} 
			Image image = new Image(path,false,Image.FILTER_NEAREST,trans);
